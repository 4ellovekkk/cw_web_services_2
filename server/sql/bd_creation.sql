...
(
    ...
    action_type     int FOREIGN key REFERENCES action_types (id)
);
...
-------------------------------------------------------cascade triggers-------------------------------------------------------
CREATE TRIGGER trg_currency_convertion_delete
    ON currency
    AFTER
        DELETE AS
BEGIN
    ...
    WHERE currncy1 IN (SELECT id
                       FROM deleted)
       OR currency2 IN (SELECT id
                        FROM deleted);
...
CREATE TRIGGER trg_currency_convertion_update
    ON currency
    AFTER
        UPDATE
    AS
BEGIN
    UPDATE
        currency_convertion
    SET currncy1 = inserted.id
    FROM inserted
    WHERE currency_convertion.currncy1 = inserted.id;

    UPDATE
        currency_convertion
    SET currency2 = inserted.id
    FROM inserted
    WHERE currency_convertion.currency2 = inserted.id;
...
CREATE TRIGGER trg_accounts_delete_account_types
    ON account_types
    AFTER
        DELETE AS
BEGIN
    DELETE
    FROM accounts
    WHERE account_type IN (SELECT id
                           FROM deleted);
...
CREATE TRIGGER trg_accounts_delete_currency
    ON currency
    AFTER
        DELETE AS
BEGIN
...
BEGIN
    UPDATE
        accounts
    SET owner_id = inserted.id
    FROM inserted
    WHERE accounts.owner_id = inserted.id;
...
CREATE TRIGGER trg_accounts_update_account_types
    ON account_types
    AFTER
        UPDATE
    AS
BEGIN
    UPDATE
        accounts
    SET account_type = inserted.id
    FROM inserted
    WHERE accounts.account_type = inserted.id;
...
CREATE TRIGGER trg_accounts_update_currency
    ON currency
    AFTER
        UPDATE
    AS
BEGIN
    UPDATE
        accounts
    SET currency = inserted.id
    FROM inserted
    WHERE accounts.currency = inserted.id;
...
alter table deposit_conditioins
    add unique (deposit_condition_name);
